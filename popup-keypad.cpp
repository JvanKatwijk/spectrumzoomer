...
//	A simple keypad for the receiver. 13 buttons and
//	an LCD display
	keyPad::keyPad (Viewer *vw) {
	myViewer	= vw;
	...
	connect (this, SIGNAL (newFrequency (qint64)),
	         myViewer, SLOT (newFrequency (qint64)));
//
...
void	keyPad::collectData	(int id) {
	if (0 <= id && id < 10) {
	   panel = 10 * panel + id;
	   theDisplay	-> display ((double)panel);
	}
	else
	if (id == 101) {		// Khz button
	   newFrequency ((double)(KHz (panel)));
	   panel = 0;
	   hidePad ();
	}
	else
	if (id == 102) {		// Mhz button
	   newFrequency ((double)(MHz (panel)));
	   panel = 0;
	   hidePad ();
	}
	else
